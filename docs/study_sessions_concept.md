...
## 4. Suggested Database Schema Evolution (Supabase/PostgreSQL)

Here is how the current database schema can be modified or extended:

```sql
-- 1. Check or create the study session status enum
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'session_status') THEN
        CREATE TYPE session_status AS ENUM ('running', 'paused', 'finished', 'cancelled');
    END IF;
END $$;

-- 2. Enhance the study_sessions table
ALTER TABLE public.study_sessions 
  ADD COLUMN IF NOT EXISTS subject VARCHAR(100),
  ADD COLUMN IF NOT EXISTS target_duration INTEGER, -- target duration in seconds
  ADD COLUMN IF NOT EXISTS pause_logs JSONB DEFAULT '[]'::jsonb, -- stores [{paused_at, resumed_at}]
...
-- 3. Ensure the participants table has status tracking
ALTER TABLE public.session_participants
  ADD COLUMN IF NOT EXISTS role VARCHAR(20) DEFAULT 'participant', -- 'host' or 'participant'
  ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'active'; -- 'active', 'away', 'left'
```
