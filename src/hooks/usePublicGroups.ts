...
export function usePublicGroups(groupEnrollments: Record<string, boolean> = {}, onUpdateEnrollment?: (groupId: string, enrolled: boolean) => void) {
  const { user } = useAuth();
  const [availableGroups, setAvailableGroups] = useState<PublicGroupInfo[]>([]);
